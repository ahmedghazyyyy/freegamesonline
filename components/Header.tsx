...
export const Header: React.FC<HeaderProps> = ({ onPlay, games }) => {
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative" ref={langRef}>
              <button 
                ...
                className="flex items-center gap-2 px-3 h-10 rounded-full bg-white/5 text-slate-400 hover:bg-brand-500 hover:text-white transition-all border border-white/10"
                title="Change Language"
                aria-label="Change Language"
              >
                <i className="fa-solid fa-globe"></i>
                <span className="text-xs font-bold uppercase hidden sm:inline-block">{i18n.language.split('-')[0]}</span>
                <i className={`fa-solid fa-chevron-down text-[10px] transition-transform duration-300 ${isLangOpen ? 'rotate-180' : ''}`}></i>
              </button>
              
              {isLangOpen && (
                <div className="absolute top-full right-0 mt-3 w-56 bg-gaming-surface border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-[110] animate-fade-in-up max-h-80 overflow-y-auto scrollbar-hide ring-1 ring-white/5">
                      ...
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 rounded-xl text-sm transition-all flex items-center justify-between group ${
                          i18n.language === lang.code 
                            ? 'bg-brand-500 text-white font-bold shadow-neon-sm' 
                            : 'text-slate-300 hover:bg-white/10 hover:text-white'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <span className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-black uppercase ${i18n.language === lang.code ? 'bg-white/20' : 'bg-white/5 group-hover:bg-white/10'}`}>
                            {lang.code}
                          </span>
                          <span>{lang.name}</span>
                        </div>
